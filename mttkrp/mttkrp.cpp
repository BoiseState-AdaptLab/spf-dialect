...
public:
  explicit Walker(
    ...
    // created.
    zero = builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(),
                                                        0);
    one = builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(),
                                                       1);
  }
  ...
  void walkLoop(omega::CG_loop *loop) {
    ...
    if (loop->needLoop_) {
      ...
      mlir::scf::ForOp forOp = builder.create<mlir::scf::ForOp>(
          builder.getUnknownLoc(), zero, m[upper_bound], one);

      // TODO: when we're actually calling the statement we will need to be
      // storing these off somehow.
...
int main(int argc, char **argv) {
  // this one is dense http://tensor-compiler.org/docs/data_analytics
  ...
  Stmt *s1 = new Stmt("A(i,j) += B(z)*D(l,j)*C(k,j)",
                      "{[z,i,k,l,j] :  0<=z<NNZ and i=UFi(z) and "
                      "k=UFk(z) and l=UFl(z) and 0<=j<J}",
                      "{[z,i,k,l,j]->[z,i,k,l,j]}", dataReads, dataWrites);
  ...
  m["K"] =
      builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(), 20);
  m["L"] =
      builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(), 30);
  m["J"] =
      builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(), 40);
  ...
  builder.setInsertionPointToEnd(theModule.getBody());
  m["NNZ"] = builder.create<mlir::arith::ConstantIndexOp>(
      builder.getUnknownLoc(), 100);
  m["J"] =
      builder.create<mlir::arith::ConstantIndexOp>(builder.getUnknownLoc(), 10);
