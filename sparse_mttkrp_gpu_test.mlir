module {
	...
	func.func @main() {
		...
		%b_values = call @values_gpu(%storage) : (!llvm.ptr<i8>) -> (memref<?xf64>)
		...
		%d_c = gpu.alloc(%K, %J) : memref<?x?xf64>
		gpu.memcpy %d_c, %c : memref<?x?xf64>, memref<?x?xf64>
		...
		%d_d = gpu.alloc(%L, %J) : memref<?x?xf64>
		gpu.memcpy %d_d, %d : memref<?x?xf64>, memref<?x?xf64>
