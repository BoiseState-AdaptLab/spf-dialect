      ...
      for (omega::EQ_Iterator eq_conj(bounds.single_conjunct()->EQs()); eq_conj;
        ...
        for (omega::Constr_Vars_Iter var(*eq_conj); var; var++) {
          ...
          if (var.curr_var()->kind() == omega::Global_Var &&
            ...
            // generated induction variables and arguments to the operation. An
            // example is given below:
            //
            // "standalone.bar"(%argb, %argc, %argd, %arga) ({
            // //this region would be the statement
            // }, {
            // ^bb0(%argb_uf0: memref<?x?x?xf64>, %argc_uf0: memref<?x?xf64>,
            //      %argd_uf0: memref<?x?xf64>, %argd_uf0: memref<?x?xf64>,
            //      %z index) :
            //  // This region is UF 0, it's expected to have the same arguments
            //  // as the operation but can also take any induction variables
            //  // generated by the time it's called.
            //
            // // This UF generates %some_variable induction variable.
            // "standalone.yield"(%some_variable) : (index) -> ()
            // })  {
            //        // attributes go here
            //     } : (memref<?x?x?xf64>, memref<?x?xf64>, memref<?x?xf64>,
            //     memref<?x?xf64>) -> ()
            //
            // The UF in question needs a mapping from the 4 memref arguments to
            // the operation and %z, which is expected to be a generated
            // induction variables.
            SmallVector<Value> potentialUfArguments = barOp.getInputOperands();
            potentialUfArguments.insert(potentialUfArguments.end(), ivs.begin(),
                                        ivs.end());
            BlockAndValueMapping map; // holds a mapping between values.
            map.map(/*from*/ ufBlock.getArguments(),
                    /*to*/ potentialUfArguments);

            // This inlines the region with the mapping we created above.
